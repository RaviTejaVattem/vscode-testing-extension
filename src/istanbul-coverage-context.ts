...
export class IstanbulCoverageContext {
	...
	public async applyJson(
		...
		await Promise.all(
			Object.values(files).map(async (entry: FileCoverageData) => {
				...
				if (originalUri) {
					run.addCoverage(
						new IstanbulFileCoverage(originalUri, entry, compiledUri, options)
					);
				}
			})
		);
	 ...
	 */
	public readonly loadDetailedCoverage: vscode.TestRunProfile['loadDetailedCoverage'] =
		async (_testRun, file) => {
						...
						...branch.locations.map((l) =>
							l.start.line !== undefined
								...
								: // the implicit "else" case of 'if' statements are emitted as a
								  ...
								  mapRange(opts, file.compiledUri, {
										start: branch.loc.end,
										end: branch.loc.end
								  })
						)
					]).then(([loc, ...branches]) => {
				...
				todo.push(
					mapRange(opts, file.compiledUri, stmt).then((loc) => {
						if (loc) {
							details.push(
								new vscode.StatementCoverage(
									mapCount(opts, file.original.s[key]),
									loc
								)
							);
						}
						...
						if (loc) {
							details.push(
								new vscode.DeclarationCoverage(
									stmt.name,
									mapCount(opts, file.original.f[key]),
									loc
								)
							);
						}
...
const mapLocation = async (
	...
	return (
		opts.mapLocation?.(uri, position) || new vscode.Location(uri, position)
	);
};

const mapCount = (opts: IIstanbulCoverageOptions, n: number) =>
	opts.booleanCounts ? n > 0 : n;
...
class IstanbulFileCoverage extends vscode.FileCoverage {
	constructor(
		...
		super(
			uri,
			parseToSum(original.s),
			parseToSum(original.b),
			parseToSum(original.f)
		);
	}
}
